...
async function project(title: string, extra: string[] = []): Promise<string> {
  ...
  const { stdout } = await run(
    ...
    [
      ...
      // that workflow says is not what this test is measuring.
...
describe("the committed bundle, run against a fake GitHub", () => {
  it("renders a projection, changelog and all", async () => {
    ...
    // What the empty `--repo-root` buys, and the only thing that says so:
    // plain mode reports config files it finds and did not read.
    expect(out).not.toContain("release-please-config.json");
  });
